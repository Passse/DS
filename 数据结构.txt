...
void input(int s,int m,int w)
{
    ...
    for(i=0;i<s;i++)
    {
        ...
        for(j=0;j<m+w;j++)
        {
            ...
            {
                Snode_a[i].itm[j].top=3;//这里有点问题，是否应改再定义一下结构体“Top”
            }
            else if(Snode_a[i].itm[j].inum%2!=0)
            {
                Snode_a[i].itm[j].top=5;
            }
            printf("请输入获得名次人数:");
            scanf("%d",&k);
                        for(o=0;o<k;o++)
    ...
    while(kg==1)
    {
        ...
        switch(c2)
        {
            case 1:
                ...
                for(i=0;i<s;i++)
                {
                    printf("第%d名: 学校编号:%d     学校总分排名:%d\n",i+1,Snode_a[i].snum,new_a[i]);
                }
                break;
            case 2:
                for(i=0;i<s;i++)
    ...
    do
    {
        ...
        if(k==-1)
            ...
            else
            {
                ...
                for(i=0;i<5;i++)
                {
                    if(Snode_a[k].itm[L].rang[i]!=0)
                    {
                        printf("项目名次:%d\n",Snode_a[k].itm[L].rang[i]);
                    }
                }
